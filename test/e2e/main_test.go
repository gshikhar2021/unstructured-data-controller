...
func testSetup(_ context.Context, runningProcesses *[]exec.Cmd, config *envconf.Config) error {
	...
	var secretCreateCmd string

	if privateKeyContent != "" {
		log.Println("Using private key from SNOWFLAKE_PRIVATE_KEY environment variable")
		secretCreateCmd = fmt.Sprintf("kubectl create secret generic %s -n %s --from-literal=privateKey='%s'",
			snowflakeSecretName, testNamespace, privateKeyContent)
	} else {
		secretFile := os.Getenv("SNOWFLAKE_SECRET_FILE")
		if secretFile == "" {
			return fmt.Errorf("Local testing provide file path")
		}

		log.Printf("Using private key from file: %s", secretFile)
	...
	if p := utils.RunCommand(fmt.Sprintf("kubectl apply -n %s -f config/samples/aws-secret.yaml", testNamespace)); p.Err() != nil {
		log.Printf("Failed to create aws-secret: %s", p.Err())
		return p.Err()
	}
